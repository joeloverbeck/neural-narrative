...
def main():
    ...
    prompt += interface_manager.prompt_for_input("Enter your notion of what the portrait should show: ")

    generated_image_product = OpenAIGeneratedImageFactory(OpenAILlmClientFactory().create_llm_client()).generate_image(
        prompt)
    ...
    url_content_product = ConcreteUrlContentFactory().get_url(generated_image_product.get())

    if not url_content_product.is_valid():
        print(f"Was unable to get the content of the url: {url_content_product.get_error()}")
        sys.exit()

    # We have a valid image.
    image_name = interface_manager.prompt_for_input("What should be the name of the image?: ")

    FilesystemManager().write_binary_file(f"data/images/{image_name}.png", url_content_product.get())
