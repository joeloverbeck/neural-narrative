...
class ConcreteUrlContentFactory(UrlContentFactory):
    ...
    def get_url(self, url: str) -> UrlContentProduct:
        ...
        if image_response.status_code == REQUEST_OK:
            return ConcreteUrlContentProduct(image_response.content,
                                             is_valid=True)
        else:
            return ConcreteUrlContentProduct(None, is_valid=False, error=
            f"Was unable to retrieve the contents of url '{url}': {image_response.status_code}"
                                             )
