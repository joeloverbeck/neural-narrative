...
class RequestsManager:
    def __init__(
        ...
        self._xtts_data = self._filesystem_manager.load_existing_or_new_json_file(
            XTTS_CONFIG_FILE
        )
    ...
    def _get_pod_url(self) -> Optional[str]:
        ...
        # Assuming you want the first available running pod
        for pod in pods:
            pod_id = pod.get("id")
    ...
    def _is_pod_ready(self) -> bool:
        ...
        if not pod_url:
            # If you can't even get the pod url, clearly it's not ready.
            return False

        # At this point the pod is technically active, but it could return 404s.
        xtts_set_tts_settings = f"{pod_url}/set_tts_settings"

        try:
            response = requests.post(xtts_set_tts_settings, json=self._xtts_data)
