...
class GenerateStoryUniverseCommand(Command):
    ...
    def execute(self) -> None:
        ...
        # At this point, the product is valid, so we store the new story universe.
        story_universes_file = self._filesystem_manager.load_existing_or_new_json_file(
            STORY_UNIVERSES_TEMPLATE_FILE
        )

        story_universes_file[product.get_name().value] = {
            "description": product.get_description().value,
            "categories": [category.value for category in product.get_categories()],
        }
        ...
        logger.info("Saved story universe '%s'.", product.get_name().value)
