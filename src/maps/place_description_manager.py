...
class PlaceDescriptionManager:
    ...
    def get_place_description(self, place_identifier: RequiredString) -> str:
        place = self._place_manager_factory.create_place_manager().get_place(
            place_identifier
        )
        place_type = (
            self._place_manager_factory.create_place_manager().determine_place_type(
                place_identifier
            )
        )
        ...
        if not place_description:
            raise ValueError(
                f"The description for place '{place_identifier.value}' was empty."
            )
