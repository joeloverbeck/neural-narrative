...
class WeathersManager:
    ...
    def get_all_weather_identifiers(self) -> [List[RequiredString]]:
        return [
            RequiredString(key) for key, value in self._load_weathers_file().items()
        ]

    def get_current_weather_identifier(self) -> RequiredString:
        ...
        return RequiredString(area_data["weather_identifier"])

    def get_weather_description(
        self, weather_identifier: RequiredString
    ) -> RequiredString:
        return RequiredString(
            self._load_weathers_file()[weather_identifier.value]["description"]
        )
