...
class PlaceSelectionManager:
    ...
    def get_available_location_types(
        self, current_area_template: RequiredString
    ) -> List[RequiredString]:
        place_categories = (
    ...
    def filter_places_by_categories(
        ...
        father_place_categories: List[RequiredString],
        ...
        for name, data in place_templates.items():
            ...
            # Match categories
            if not any(
                category.value in place_categories
                for category in father_place_categories
            ):
                continue

            # If location_type is specified, check if it matches
            if location_type and place_type != location_type.value:
                continue
    ...
    @staticmethod
    def select_random_place(matching_places: Dict) -> RequiredString:
        ...
        return RequiredString(random.choice(list(matching_places.keys())))
