...
from src.maps.abstracts.factory_products import RandomTemplateTypeMapEntryCreationResult, PlaceTemplateProduct
...
class ConcreteRandomTemplateTypeMapEntryProvider(
    RandomTemplateTypeMapEntryProvider):

    def __init__(self, config: RandomTemplateTypeMapEntryProviderConfig,
        ...
        self._templates_repository = (templates_repository or
                                      TemplatesRepository(self._filesystem_manager))

    def _create_template_product(self, available_templates: Dict
                                 ) -> PlaceTemplateProduct:
        categories = (self._factories_config.place_manager_factory.
                      create_place_manager().get_place_categories(self._config.
                                                                  father_template, self._config.father_place_type))
        return (self._factories_config.
        random_place_template_based_on_categories_factory.
        create_random_place_template_based_on_categories(
            available_templates, categories))

    def create_random_place_type_map_entry(self
                                           ) -> RandomTemplateTypeMapEntryCreationResult:
        try:
            available_templates = self._templates_repository.load_template(self
                                                                           ._config.place_type)
            ...
            template_product = self._create_template_product(
                available_templates)
            ...
            ).create_command(template_product.get()).execute()
            return ConcreteRandomTemplateTypeMapEntryCreationResult(
