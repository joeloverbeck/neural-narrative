...
class ConfigManager:
    ...
    def get_heavy_llm(self) -> str:
        ...
        if config_data['heavy_llm'].lower() == 'hermes_405b':
            ...
            raise ValueError(
                f"Heavy LLM named '{config_data['heavy_llm']}' unhandled.")

    def get_light_llm(self) -> str:
        config_data = self._filesystem_manager.load_existing_or_new_json_file(
            CONFIG_FILE)
        if config_data['light_llm'].lower() == 'hermes_70b':
            ...
            raise ValueError(
                f"Light LLM named '{config_data['light_llm']}' unhandled.")
