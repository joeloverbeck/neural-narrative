...
from src.constants import AREA_GENERATION_PROMPT_FILE, REGION_GENERATION_PROMPT_FILE, LOCATION_GENERATION_PROMPT_FILE, \
...
class PlaceGenerationToolResponseFactory(ToolResponseFactory):
    ...
    def create_llm_response(self) -> LlmToolResponseProduct:
        ...
        if self._template_type == TemplateType.REGION:
            place_generation_prompt = filesystem_manager.read_file(REGION_GENERATION_PROMPT_FILE)

        elif self._template_type == TemplateType.AREA:
            place_generation_prompt = filesystem_manager.read_file(AREA_GENERATION_PROMPT_FILE)

        elif self._template_type == TemplateType.LOCATION:
            place_generation_prompt = filesystem_manager.read_file(LOCATION_GENERATION_PROMPT_FILE)
        else:
        ...
        if self._template_type == TemplateType.REGION:
            current_place_type_templates = filesystem_manager.load_existing_or_new_json_file(
                filesystem_manager.get_file_path_to_regions_template_file())
        elif self._template_type == TemplateType.AREA:
            current_place_type_templates = filesystem_manager.load_existing_or_new_json_file(
                filesystem_manager.get_file_path_to_areas_template_file())
