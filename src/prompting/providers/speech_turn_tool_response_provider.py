...
class SpeechTurnChoiceToolResponseProvider(ToolResponseProvider):
    ...
    def create_llm_response(self) -> LlmToolResponseProduct:
        character_choice_dialogue_initial_prompting_messages_product = (self
                                                                        .
                                                                        _character_choice_dialogue_initial_prompting_messages_provider_factory
                                                                        .
                                                                        create_character_choice_dialogue_initial_prompting_messages_provider
                                                                        (self._participants, self._transcription).
                                                                        create_initial_prompting_messages())
        llm_content_product = (self._llm_content_provider_factory.
                               create_llm_content_provider_factory(
            character_choice_dialogue_initial_prompting_messages_product).
                               generate_content())
        if not llm_content_product.is_valid():
            return ConcreteLlmToolResponseProduct(llm_response={}, is_valid
            =False, error=
                                                  f'LLM failed to produce a response: {llm_content_product.get_error()}'
                                                  )
        parsed_tool_response = (self.
                                _handle_parsed_tool_response_for_dialogue_character_choice_strategy_factory
                                .
                                create_handle_parsed_tool_response_for_dialogue_character_choice_strategy
                                ().handle_parsed_tool_response(llm_content_product))
        if not parsed_tool_response.is_valid():
            raise ValueError(
                f'Failed to choose the next speaker: {parsed_tool_response.get_error()}'
            )
        parsed_tool_response.get()['voice_model'] = Character(self.
                                                              _playthrough_name,
