...
class CharacterToolResponseDataExtractionProvider(ToolResponseDataExtractionProvider):

    def __init__(self, parsed_tool_response: dict):
        if not isinstance(parsed_tool_response, dict):
            raise TypeError(
                f"parsed_tool_response should be a dict. It was '{type(parsed_tool_response)}'."
            )
