...
class FunctionCallSanitizer:
    ...
    REMOVE_EXTRA_CLOSING_BRACES_BEFORE_CLOSING_FUNCTION_TAG_REGEX = re.compile(r'}+</function>')
    REMOVE_SPACES_BETWEEN_SYMBOL_OF_FUNCTION_TAG_AND_SYMBOL_OF_JSON_OBJECT_REGEX = re.compile(r'>\s+{')
    REMOVE_SPACES_BETWEEN_THE_CLOSING_FUNCTION_TACT_REGEX = re.compile(r'\s+</function>')
    REPLACE_SINGLE_QUOTES_WITH_DOUBLE_QUOTES_REGEX = re.compile(r"(?<=[:\[{,])\s*'([^']*)'\s*(?=[,}\]])")
    INSERT_LINE_BREAK_AFTER_PERIOD_WITH_NO_SPACE_FOLLOWED_BY_LETTER_REGEX = re.compile(r'\.(?=\w)')
    ...
    def sanitize(self) -> str:
        ...
        """
        function_call = self._function_call.strip()  # Step 1: Remove leading/trailing spaces and newlines
        ...
        function_call = re.sub(FunctionCallSanitizer.REMOVE_EXTRA_SPACES_BETWEEN_FUNCTION_TAGS_ONE_REGEX,
                               "<function", function_call)
        function_call = re.sub(FunctionCallSanitizer.REMOVE_EXTRA_SPACES_BETWEEN_FUNCTION_TAGS_TWO_REGEX,
                               "</function>", function_call)
        ...
        if function_call.endswith('/function>') and not function_call.endswith('</function>'):
        ...
        function_call = re.sub(FunctionCallSanitizer.REMOVE_SPACES_BETWEEN_THE_CLOSING_FUNCTION_TACT_REGEX,
                               '</function>', function_call)
        ...
        function_call = re.sub(FunctionCallSanitizer.REPLACE_SINGLE_QUOTES_WITH_DOUBLE_QUOTES_REGEX, r'"\1"',
                               function_call)
