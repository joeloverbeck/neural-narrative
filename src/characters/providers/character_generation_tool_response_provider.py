...
from src.base.constants import CHARACTER_GENERATOR_TOOL_FILE, CHARACTER_GENERATION_INSTRUCTIONS_FILE, \
    WORLDS_TEMPLATES_FILE, LOCATIONS_TEMPLATES_FILE, AREAS_TEMPLATES_FILE, REGIONS_TEMPLATES_FILE, VOICE_GENDERS, \
    VOICE_AGES, VOICE_EMOTIONS, VOICE_TEMPOS, VOICE_VOLUMES, VOICE_TEXTURES, VOICE_TONES, VOICE_STYLES, \
    VOICE_PERSONALITIES, VOICE_SPECIAL_EFFECTS
...
class CharacterGenerationToolResponseProvider(BaseToolResponseProvider,
                                              ToolResponseProvider):

    def __init__(self, playthrough_name: str, places_parameter:
    PlacesTemplatesParameter, produce_tool_response_strategy_factory:
    ProduceToolResponseStrategyFactory,
                 ...
                 CharacterGenerationInstructionsFormatterFactory, filesystem_manager:
            Optional[FilesystemManager] = None, characters_manager: Optional[
    ...
    def _read_and_format_tool_file(self, tool_file: str) -> dict:
        template_copy = (self._filesystem_manager.
                         load_existing_or_new_json_file(tool_file))
        try:
            template_copy['function']['parameters']['properties'][
                'voice_gender']['enum'] = VOICE_GENDERS
            template_copy['function']['parameters']['properties']['voice_age'][
                'enum'] = VOICE_AGES
            template_copy['function']['parameters']['properties'][
                'voice_emotion']['enum'] = VOICE_EMOTIONS
            template_copy['function']['parameters']['properties']['voice_tempo'
            ]['enum'] = VOICE_TEMPOS
            template_copy['function']['parameters']['properties'][
                'voice_volume']['enum'] = VOICE_VOLUMES
            template_copy['function']['parameters']['properties'][
                'voice_texture']['enum'] = VOICE_TEXTURES
            template_copy['function']['parameters']['properties']['voice_tone'
            ]['enum'] = VOICE_TONES
            template_copy['function']['parameters']['properties']['voice_style'
            ]['enum'] = VOICE_STYLES
            template_copy['function']['parameters']['properties'][
                'voice_personality']['enum'] = VOICE_PERSONALITIES
            template_copy['function']['parameters']['properties'][
                'voice_special_effects']['enum'] = VOICE_SPECIAL_EFFECTS
    ...
    def get_formatted_prompt(self) -> str:
        ...
        instructions = (self.
                        _character_generation_instructions_formatter_factory.
                        create_formatter(templates).format())
        return instructions
    ...
    def get_user_content(self) -> str:
        user_content_product = (self.
                                _user_content_for_character_generation_factory.
                                create_user_content_for_character_generation())
    ...
    def create_llm_response(self) -> LlmToolResponseProduct:
        try:
            ...
            capture_traceback()
            return ConcreteLlmToolResponseProduct({}, is_valid=False, error
            =f'An error occurred while creating the LLM response: {e}')

    def _load_templates(self) -> dict:
        """Loads all necessary templates and metadata from the filesystem."""
        playthrough_metadata = (self._filesystem_manager.
                                load_existing_or_new_json_file(self._filesystem_manager.
                                                               get_file_path_to_playthrough_metadata(
            self._playthrough_name)))
        worlds_templates = (self._filesystem_manager.
                            load_existing_or_new_json_file(WORLDS_TEMPLATES_FILE))
        regions_templates = (self._filesystem_manager.
                             load_existing_or_new_json_file(REGIONS_TEMPLATES_FILE))
        areas_templates = (self._filesystem_manager.
                           load_existing_or_new_json_file(AREAS_TEMPLATES_FILE))
        locations_templates = (self._filesystem_manager.
                               load_existing_or_new_json_file(LOCATIONS_TEMPLATES_FILE))
        character_generation_instructions = self._filesystem_manager.read_file(
            CHARACTER_GENERATION_INSTRUCTIONS_FILE)
        return {'playthrough_metadata': playthrough_metadata,
                'worlds_templates': worlds_templates, 'regions_templates':
                    regions_templates, 'areas_templates': areas_templates,
    ...
    def _get_location_details(self, locations_templates: dict) -> tuple:
        ...
        if location_template:
            ...
            location_description = locations_templates[location_template][
                'description']
