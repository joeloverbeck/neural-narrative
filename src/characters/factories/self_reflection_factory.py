...
from src.base.constants import SELF_REFLECTION_GENERATION_PROMPT_FILE, SELF_REFLECTION_GENERATION_TOOL_FILE
...
class SelfReflectionFactory(BaseToolResponseProvider):
    ...
    def __init__(self, playthrough_name: str, character_identifier: str,
                 ...
                 ProduceToolResponseStrategyFactory, character_information_factory:
            CharacterInformationProvider, filesystem_manager: Optional[
                FilesystemManager] = None, characters_manager: Optional[
    ...
    def get_user_content(self) -> str:
        return (
            'Write a meaningful and compelling self-reflection from the first-person perspective of the character regarding their memories. Follow the provided instructions.'
        )

    def create_product(self, arguments: dict):
        return SelfReflectionProduct(arguments.get('self_reflection'),
                                     is_valid=True)

    def get_prompt_kwargs(self) -> dict:
        character = Character(self._playthrough_name, self.
                              _character_identifier)
        return {'name': character.name, 'character_information': self.
        _character_information_factory.get_information()}
