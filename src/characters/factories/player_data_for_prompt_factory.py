...
class PlayerDataForPromptFactory:
    ...
    CharacterFactory, playthrough_manager: Optional[PlaythroughManager]
                 = None, character_memories: Optional[CharacterMemories] = None):
    ...
    def create_player_data_for_prompt(self) -> PlayerDataForPrompt:
        ...
        if memories:
            player_memories = [memory.strip() for memory in memories.strip(
            ).split('\n') if memory.strip()]
        else:
            player_memories = []
        return PlayerDataForPrompt({'player_name': player.name,
                                    'player_description': player.description, 'player_personality':
                                        player.personality, 'player_profile': player.profile,
                                    'player_likes': player.likes, 'player_dislikes': player.
                                   dislikes, 'player_secrets': player.secrets,
                                    'player_speech_patterns': player.speech_patterns,
                                    'player_health': player.health, 'player_equipment': player.
                                   equipment}, player_memories)
