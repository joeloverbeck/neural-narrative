...
class StoreCharacterMemoryCommand(Command):
    def __init__(self, playthrough_name: str, character_identifier: str, memory: str,
        ...
        if not isinstance(character_identifier, str):
            raise TypeError(f"character_identifier should be a string, but it was {type(character_identifier)}.")
    ...
    def execute(self) -> None:
        character_data = self._characters_manager.load_character_data(self._character_identifier)

        file_path = self._filesystem_manager.get_file_path_to_character_memories(self._playthrough_name,
