...
from src.base.commands.create_playthrough_metadata_command import CreatePlaythroughMetadataCommand
from src.characters.commands.generate_player_character_command import GeneratePlayerCharacterCommand
from src.characters.factories.generate_character_command_factory_composer import GenerateCharacterCommandFactoryComposer
...
class PlaythroughService:
    ...
    @staticmethod
    def create_playthrough(playthrough_name: str, story_universe_template:
    str, player_notion: str):
        ...
        generate_character_command_factory = (
            GenerateCharacterCommandFactoryComposer(playthrough_name).
            compose_factory())
        ...
        generate_player_character_command = GeneratePlayerCharacterCommand(
            playthrough_name, player_notion,
            generate_character_command_factory, hierarchy_manager_factory)
        visit_place_command_factory = VisitPlaceCommandFactoryComposer(
            playthrough_name).compose_factory()
        CreatePlaythroughCommand(create_playthrough_metadata_command,
                                 create_initial_map_command, generate_player_character_command,
                                 visit_place_command_factory, map_manager_factory).execute()
