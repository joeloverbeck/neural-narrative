...
class ConsoleInterfaceManager:
    ...
    def create_places_templates_parameter_for_character_generation(
        ...
        world_template = RequiredString(
            self.prompt_for_input("To what world does the character belong?: ")
        )

        PrintPlaceTemplateListCommand(TemplateType.REGION).execute()

        region_template = RequiredString(
            self.prompt_for_input("To what region does the character belong?: ")
        )

        PrintPlaceTemplateListCommand(TemplateType.AREA).execute()

        area_template = RequiredString(
            self.prompt_for_input("To what area does the character belong?: ")
        )

        PrintPlaceTemplateListCommand(TemplateType.LOCATION).execute()

        location_template = RequiredString(
            input("To what location does the character belong? (could be empty): ")
        )
