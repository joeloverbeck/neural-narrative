...
from src.actions.algorithms.produce_action_resolution_algorithm import ProduceActionResolutionAlgorithm
...
from src.actions.algorithms.store_action_resolution_algorithm import StoreActionResolutionAlgorithm
...
def action_view(action_name, action_icon, action_endpoint, prompt_file,
    ...
    if request.method == 'GET':
        ...
        current_place = MapManagerFactory(playthrough_name).create_map_manager(
        ).get_current_place_template()
        return render_template('action.html', action_name=action_name,
                               action_icon=action_icon, current_place=current_place,
                               action_endpoint=action_endpoint)
    else:
        playthrough_name = session.get('playthrough_name')
        if not playthrough_name:
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return jsonify(success=False, error='Playthrough not found.'
                               ), 400
            else:
                return redirect(url_for('index'))
        ...
        if form_type == 'resolve_action':
            ...
            if not action_goal:
                ...
                if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                    ...
                    f'Please enter a {action_name.lower()} goal.'), 400
                else:
                    flash(f'Please enter a {action_name.lower()} goal.',
                          'error')
            ...
            action_resolution_factory = ActionResolutionFactory(
                playthrough_name=playthrough_name, action_name=action_name,
                ...
                compose_provider(),
                players_and_followers_information_factory=
                players_and_followers_information_factory, prompt_file=
                prompt_file, tool_file=tool_file)
            store_character_memory_command_factory = (
                StoreCharacterMemoryCommandFactory(playthrough_name))
            participants = ParticipantsManager(playthrough_name
                                               ).initialize_participants()
            store_action_resolution_algorithm = StoreActionResolutionAlgorithm(
                playthrough_name, participants,
                store_character_memory_command_factory)
            produce_voice_lines_for_action_resolution_algorithm = (
                ProduceVoiceLinesForActionResolutionAlgorithm(
                    DirectVoiceLineGenerationAlgorithmFactory()))
            action_resolution_algorithm = ProduceActionResolutionAlgorithm(
                playthrough_name, action_resolution_factory,
            ...
            try:
                ...
                if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                    ...
                    flash(str(e), 'error')
                    return redirect(url_for(action_endpoint))
            current_place = MapManagerFactory(playthrough_name
                                              ).create_map_manager().get_current_place_template()
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                response_data = {'success': True, 'message':
                    f'{action_name} resolved successfully.', 'action_name':
                                     action_name, 'result': {'narrative': result.
                get_narrative(), 'outcome': result.get_outcome(),
                                                             'narrative_voice_line_url': result.
                                                             get_narrative_voice_line_url(),
                                                             'outcome_voice_line_url': result.
                                                             get_outcome_voice_line_url()}, 'current_place':
                                     current_place, 'characters': [{'identifier': char.
                identifier, 'name': char.name, 'description': char.
                description, 'equipment': char.equipment, 'health':
                                                                        char.health} for char in character_list],
                                 'form_action':
                                     url_for(action_endpoint)}
                return jsonify(response_data), 200
            else:
                return render_template(f'{action_endpoint}.html',
                                       current_place=current_place, result=result, characters=
                                       character_list)
        elif form_type == 'modify_characters':
            characters_manager = CharactersManager(playthrough_name)
            playthrough_manager = PlaythroughManager(playthrough_name)
            player_identifier = playthrough_manager.get_player_identifier()
            character_ids = [player_identifier] + [follower.identifier for
                                                   follower in characters_manager.get_followers()]
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                'Character changes saved successfully.'), 200
            else:
