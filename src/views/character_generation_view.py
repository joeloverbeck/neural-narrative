...
class CharacterGenerationView(MethodView):
    ...
    def get(self):
        ...
        places_templates_parameter = HierarchyManagerFactory(playthrough_name
                                                             ).create_hierarchy_manager().fill_places_templates_parameter(
        ...
        character_generation_guidelines = (filesystem_manager.
        load_existing_or_new_json_file(
            CHARACTER_GENERATION_GUIDELINES_FILE))
        if not character_guidelines_manager.create_key(playthrough_manager.
                                                               get_story_universe_template(), world_template,
                                                       region_template,
                                                       area_template, location_template
                                                       ) in character_generation_guidelines:
            hierarchy_manager_factory = HierarchyManagerFactory(
                playthrough_name)
            GenerateCharacterGenerationGuidelinesAlgorithm(playthrough_name,
                                                           playthrough_manager.get_current_place_identifier(),
                CharacterGenerationGuidelinesProviderFactoryComposer(
                    playthrough_name).compose_factory(), hierarchy_manager_factory
                                                           ).do_algorithm()
        ...
        character_generation_message = session.pop(
            'character_generation_message', None)
        return render_template('character-generation.html', guidelines=[
            guideline for guideline in guidelines], selected_guideline=
                               session.get('selected_guideline', ''),
    ...
    def handle_generate_character(playthrough_name):
        ...
        try:
            CharacterService.generate_character(playthrough_name,
                                                guideline_text)
            latest_identifier = CharactersManager(playthrough_name
                                                  ).get_latest_character_identifier()
            response = {'success': True, 'message':
                f"Character '{Character(playthrough_name, latest_identifier).name}' generated successfully."
                        }
        except CharacterGenerationError as e:
            logger.error('Failed to generate character. Error: %s', e)
            response = {'success': False, 'error':
                f'Character generation failed. Error: {str(e)}'}
        except Exception as e:
            capture_traceback()
            logger.error('Unspecified error. Error: %s', e)
            response = {'success': False, 'error':
                f'Unspecified error: {str(e)}'}
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return jsonify(response)
        else:
            session['character_generation_message'] = response.get('message'
    ...
    def handle_generate_guidelines(playthrough_name):
        try:
            playthrough_manager = PlaythroughManager(playthrough_name)
            place_identifier = (playthrough_manager.
                                get_current_place_identifier())
            hierarchy_manager_factory = HierarchyManagerFactory(
                playthrough_name)
            product = GenerateCharacterGenerationGuidelinesAlgorithm(
                playthrough_name, place_identifier,
                CharacterGenerationGuidelinesProviderFactoryComposer(
                    playthrough_name).compose_factory(), hierarchy_manager_factory
            ).do_algorithm()
            if not product.is_valid():
                raise ValueError('Failed to generate guidelines: ' +
                                 product.get_error())
            guidelines = product.get()
            response = {'success': True, 'message':
                'Guidelines generated successfully.', 'guidelines': [
                guideline for guideline in guidelines]}
        except Exception as e:
            logger.error('Failed to generate guidelines. Error: %s', e)
            response = {'success': False, 'error':
                f'Failed to generate guidelines. Error: {str(e)}'}
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return jsonify(response)
        else:
