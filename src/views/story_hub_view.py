...
from src.characters.factories.party_data_for_prompt_factory import PartyDataForPromptFactory
...
from src.concepts.algorithms.generate_interesting_dilemmas_algorithm import GenerateInterestingDilemmasAlgorithm
from src.concepts.algorithms.generate_interesting_situations_algorithm import GenerateInterestingSituationsAlgorithm
from src.concepts.algorithms.generate_plot_blueprints_algorithm import GeneratePlotBlueprintsAlgorithm
from src.concepts.algorithms.generate_plot_twists_algorithm import GeneratePlotTwistsAlgorithm
from src.concepts.factories.goals_factory import GoalsFactory
from src.concepts.factories.interesting_dilemmas_factory import InterestingDilemmasFactory
from src.concepts.factories.interesting_situations_factory import InterestingSituationsFactory
...
from src.maps.composers.places_descriptions_provider_composer import PlacesDescriptionsProviderComposer
...
class StoryHubView(MethodView):
    ...
    def get(self):
        ...
        items_to_load = [('plot_blueprints', filesystem_manager.
                          get_file_path_to_plot_blueprints), ('interesting_situations',
                                                              filesystem_manager.get_file_path_to_interesting_situations),
                         (
                             'interesting_dilemmas', filesystem_manager.
                             get_file_path_to_interesting_dilemmas), ('goals',
        ...
        for var_name, file_path_method in items_to_load:
            ...
            data[var_name] = [line for line in filesystem_manager.
            read_file_lines(file_path)]
        facts_file_path = filesystem_manager.get_file_path_to_facts(
            playthrough_name)
    ...
    def post(self):
        ...
        places_descriptions_provider = PlacesDescriptionsProviderComposer(
            playthrough_name).compose_provider()
        if action.startswith('generate_'):
            action_name = action[len('generate_'):]
            generate_action_mapping = {'plot_blueprints': {'factory_class':
                                                               PlotBlueprintsFactory, 'algorithm_class':
                                                               GeneratePlotBlueprintsAlgorithm, 'response_key':
                                                               'plot_blueprints', 'factory_args': [playthrough_name_obj,
                                                                                                   produce_tool_response_strategy_factory,
                                       ...
                                       'situations': {
                                           ...
                                           'response_key': 'interesting_situations', 'factory_args': [
                                               playthrough_name_obj,
                                               produce_tool_response_strategy_factory,
                                               places_descriptions_provider,
                                               player_and_followers_information_factory]}, 'dilemmas': {
                    'factory_class': InterestingDilemmasFactory,
                    'algorithm_class': GenerateInterestingDilemmasAlgorithm,
                    'response_key': 'interesting_dilemmas', 'factory_args': [
                        playthrough_name_obj,
                        produce_tool_response_strategy_factory,
                        places_descriptions_provider,
                        player_and_followers_information_factory]}, 'goals': {
                    'factory_class': GoalsFactory, 'algorithm_class':
                        GenerateGoalsAlgorithm, 'response_key': 'goals',
                    'factory_args': [playthrough_name_obj,
                                     produce_tool_response_strategy_factory,
                                     places_descriptions_provider,
                                     player_and_followers_information_factory]}, 'plot_twists':
                                           {'factory_class': PlotTwistsFactory, 'algorithm_class':
                                               GeneratePlotTwistsAlgorithm, 'response_key': 'plot_twists',
                                            'factory_args': [playthrough_name_obj,
                                                             produce_tool_response_strategy_factory,
                                                             places_descriptions_provider,
                                                             player_and_followers_information_factory]}}
            if action_name in generate_action_mapping:
                ...
                factory_instance = mapping['factory_class'](*mapping[
                    'factory_args'])
                ...
                try:
                    ...
                    response = {'success': True, 'message':
                        f"{mapping['response_key'].replace('_', ' ').capitalize()} generated successfully."
                        , mapping['response_key']: [item for item in items]}
                except Exception as e:
                    capture_traceback()
                    logger.error(e)
                    response = {'success': False, 'error':
                        f"Failed to generate {mapping['response_key'].replace('_', ' ')}. Error: {str(e)}"
                                }
            ...
            delete_action_mapping = {'plot_blueprint': filesystem_manager.
            get_file_path_to_plot_blueprints(playthrough_name_obj),
                                     'situation': filesystem_manager.
                                     get_file_path_to_interesting_situations(
                                         playthrough_name_obj), 'dilemma': filesystem_manager.
                get_file_path_to_interesting_dilemmas(playthrough_name_obj),
                                     'goal': filesystem_manager.get_file_path_to_goals(
                                         playthrough_name_obj), 'plot_twist': filesystem_manager.
                get_file_path_to_plot_twists(playthrough_name_obj)}
            ...
            facts = WebInterfaceManager.remove_excessive_newline_characters(
                facts)
            filesystem_manager.write_file(filesystem_manager.
                                          get_file_path_to_facts(playthrough_name), facts)
