...
class ChatView(MethodView):
    ...
    def get(self):
        ...
        if (not dialogue_participants and not playthrough_manager.
                has_ongoing_dialogue(playthrough_name)):
            logger.info(
                'There were no dialogue participants, and no ongoing dialogue.'
            )
            return redirect(url_for('participants'))
        filesystem_manager = FilesystemManager()
        ongoing_dialogue_file = (filesystem_manager.
                                 load_existing_or_new_json_file(filesystem_manager.
                                                                get_file_path_to_ongoing_dialogue(playthrough_name)))
        if not dialogue_participants:
            session['participants'] = ongoing_dialogue_file['participants']
        if not purpose and playthrough_manager.has_ongoing_dialogue(
                playthrough_name):
        ...
        return render_template('chat.html', dialogue=dialogue, current_time
        =time_manager.get_time_of_the_day(), current_place_template=
    ...
    def post(self):
        ...
        if not playthrough_name or not dialogue_participants:
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return jsonify({'success': False, 'error':
                    'Session expired. Please start a new chat.'}), 400
            else:
                return redirect(url_for('index'))
        dialogue = session.get('dialogue', [])
        ...
        if action == 'Send':
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                messages_data = []
                for msg in messages:
                    messages_data.append({'alignment': msg['alignment'],
                                          'message_text': msg['message_text'], 'sender_name':
                                              msg['sender_name'], 'sender_photo_url': msg[
                            'sender_photo_url'], 'file_url': msg['file_url'] or ''}
                                         )
            ...
            session['dialogue'] = dialogue
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                messages_data = [{'alignment': ambient_message['alignment'],
