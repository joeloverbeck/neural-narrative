...
class LoadDataFromOngoingDialogueCommand(Command):
    def __init__(self, playthrough_name: str, participants: Participants, messages_to_llm: MessagesToLlm,
                 transcription: Transcription, filesystem_manager: FilesystemManager = None):
    ...
    def execute(self) -> None:
        ...
        for identifier, participant in ongoing_dialogue_file["participants"].items():
            self._participants.add_participant(identifier, participant["name"], participant["description"],
                                               participant["personality"], participant["equipment"])
        ...
        for speech_turn in ongoing_dialogue_file["transcription"]:
            self._transcription.add_speech_line(speech_turn)
