...
from src.dialogues.composers.dialogue_turn_factory_composer import DialogueTurnFactoryComposer
...
class LaunchDialogueCommand(Command):
    ...
    def __init__(self, playthrough_name: str, player_identifier: str,
                 participants: Participants, purpose: Optional[str], messages_to_llm:
            Optional[MessagesToLlm], transcription: Optional[Transcription],
                 dialogue_observer: Observer, player_input_factory:
            PlayerInputFactory,
                 ...
                 MessageDataProducerForSpeechTurnStrategy, config_manager:
            ConfigManager = None):
    ...
    def execute(self) -> None:
        introduce_player_input_into_dialogue_command_factory = (
            IntroducePlayerInputIntoDialogueCommandFactory(self.
                                                           _playthrough_name, self._player_identifier, self.
                                                           _message_data_producer_for_introduce_player_input_into_dialogue_strategy
                                                           ))
        llm_client = OpenRouterLlmClientFactory().create_llm_client()
        involve_player_in_dialogue_strategy = (
            ConcreteInvolvePlayerInDialogueStrategy(self._player_identifier,
            self._player_input_factory,
                                                    introduce_player_input_into_dialogue_command_factory))
        dialogue_turn_factory = DialogueTurnFactoryComposer(self.
                                                            _playthrough_name, self._player_identifier,
                                                            self._participants,
                                                            self._purpose, llm_client, self._messages_to_llm, self.
                                                            _transcription, involve_player_in_dialogue_strategy, self.
                                                            _message_data_producer_for_speech_turn_strategy).compose()
        ...
        store_character_memory_command_factory = (
            StoreCharacterMemoryCommandFactory(self._playthrough_name))
        summarize_dialogue_command_factory = SummarizeDialogueCommandFactory(
            self._participants, dialogue_summary_provider_factory,
            store_character_memory_command_factory)
        store_dialogues_command_factory = StoreDialoguesCommandFactory(self
                                                                       ._playthrough_name, self._participants)
        produce_dialogue_command = ProduceDialogueCommand(self.
                                                          _playthrough_name, self._participants, self._purpose,
                                                          dialogue_turn_factory, summarize_dialogue_command_factory,
