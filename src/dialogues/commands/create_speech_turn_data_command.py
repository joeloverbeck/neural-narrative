...
class CreateSpeechTurnDataCommand(Command, Subject):
    ...
    Transcription, speech_turn_choice_response: LlmToolResponseProduct,
    ...
    def execute(self) -> None:
        speech_data_product = (self._llm_speech_data_provider_factory.
                               create_llm_speech_data_provider(self._messages_to_llm).
                               create_speech_data())
        if not speech_data_product.is_valid():
            logger.error('Failed to produce speech data: %s',
                         speech_data_product.get_error())
        ...
        self._transcription.add_speech_turn(name,
                                            f"*{speech_data_product.get()['narration_text']}* {speech_data_product.get()['speech']}"
            )
        self.notify(self._message_data_producer_for_speech_turn_strategy.
                    produce_message_data(self._speech_turn_choice_response,
                                         speech_data_product))
