...
class ConcreteDialogueTurnFactory(DialogueTurnFactorySubject):
    ...
    def __init__(self, dialogue_turn_factory_config:
    DialogueTurnFactoryConfig, dialogue_turn_factory_factories_config:
    DialogueTurnFactoryFactoriesConfig,
                 dialogue_turn_factory_strategies_config:
                 DialogueTurnFactoryStrategiesConfig, playthrough_manager:
        ...
        self._messages_to_llm = (dialogue_turn_factory_config.
                                 messages_to_llm or MessagesToLlm())
        self._transcription = (dialogue_turn_factory_config.transcription or
                               Transcription())
    ...
    def _get_player_input(self) -> PlayerInputProduct:
        ...
        return self._involve_player_in_dialogue_strategy.do_algorithm(self.
                                                                      _messages_to_llm, self._transcription)

    def _choose_next_speaker(self) -> LlmToolResponseProduct:
        """Determine the next speaker in the dialogue."""
        response_provider = (self.
                             _speech_turn_choice_tool_response_provider_factory.
                             create_speech_turn_choice_tool_response_provider(self.
                                                                              _transcription))
    ...
    def _validate_next_speaker(self, speech_turn_choice_response:
    LlmToolResponseProduct) -> None:
        ...
        if speech_turn_choice_response.get()['identifier'
        ] == self._playthrough_manager.get_player_identifier():
            raise InvalidNextSpeakerError('Next speaker cannot be the player.')

    def _process_speech_turn(self, player_input_product: PlayerInputProduct,
        ...
        user_messages_strategy = (self.
                                  _determine_user_messages_for_speech_turn_strategy_factory.
                                  create_strategy(player_input_product, self._messages_to_llm))
    ...
    def _create_dialogue_product(self, has_ended: bool) -> DialogueProduct:
        ...
        return ConcreteDialogueProduct(self._messages_to_llm, self.
                                       _transcription, has_ended=has_ended)

    def _determine_next_speaker(self) -> LlmToolResponseProduct:
        if not self._participants.has_only_two_participants_with_player(self
                                                                                ._playthrough_manager.get_player_identifier()):
            return self._choose_next_speaker()
        else:
            return ConcreteLlmToolResponseProduct(self._participants.
                                                  get_other_participant_data(self._playthrough_manager.
                                                                             get_player_identifier()), is_valid=True)
    ...
    def process_turn_of_dialogue(self) -> DialogueProduct:
        try:
            ...
            self._process_speech_turn(player_input_product,
                                      speech_turn_choice_response)
            ...
            raise DialogueProcessingError(
                'Was unable to find a file. Error: %s', e) from e
        except Exception as e:
            raise DialogueProcessingError('An unexpected error occurred: %s', e
                                          ) from e
