...
class ConcreteDialogueSummaryProvider(DialogueSummaryProvider):
    def __init__(self, llm_client: LlmClient, model: str, transcription: Transcription, max_retries: int = MAX_RETRIES):
        assert llm_client
        assert model
    ...
    def create_summary(self) -> SummaryProduct:
        ...
        llm_content_product = ConcreteLlmContentProvider(model=self._model, messages_to_llm=messages_to_llm,
