from src.base.constants import SUMMARIZE_DIALOGUE_PROMPT_FILE, DIALOGUE_SUMMARIZATION_TOOL_FILE, TOOL_INSTRUCTIONS_FILE, \
    MAX_RETRIES
...
class ConcreteDialogueSummaryProvider(DialogueSummaryProvider):
    ...
    def __init__(self, llm_client: LlmClient, model: str, transcription:
    Transcription, max_retries: int = MAX_RETRIES):
    ...
    def create_summary(self) -> SummaryProduct:
        ...
        messages_to_llm.add_message('system', filesystem_manager.read_file(
            SUMMARIZE_DIALOGUE_PROMPT_FILE) +
                                    f"""

Here's the dialogue to summarize:
{self._transcription.get_prettified_transcription()}"""
                                    + generate_tool_prompt(filesystem_manager.read_json_file(
            DIALOGUE_SUMMARIZATION_TOOL_FILE), filesystem_manager.read_file
        (TOOL_INSTRUCTIONS_FILE)))
        messages_to_llm.add_message('user',
                                    'Summarize the provided dialogue. Do not write any preamble, just do it as instructed.'
                                    )
        llm_content_product = ConcreteLlmContentProvider(model=self._model,
                                                         messages_to_llm=messages_to_llm, llm_client=self._llm_client,
                                                         max_retries=self._max_retries,
                                                         temperature=0.2).generate_content()
        if not llm_content_product.is_valid():
            return ConcreteSummaryProduct('', is_valid=False, error=
            f'The LLM failed to produce valid content: {llm_content_product.get_error()}'
                                          )
