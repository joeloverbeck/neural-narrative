...
class CharacterChoiceDialogueSystemContentForPromptProvider(
    ...
    def __init__(self, participants: Participants, player_identifier:
    Optional[str], transcription: Transcription, prompt_template: str,
                 tool_data: dict, tool_instructions_template: str):
    ...
    def create_system_content_for_prompt(self) -> SystemContentForPromptProduct:
        all_participants = '\n'.join([
            f"Identifier: {identifier} / Name: {participant['name']}" for
            identifier, participant in self._participants.get().items()])
        if self._player_identifier:
            participants_without_player = '\n'.join([
                ...
                for identifier, participant in self._participants.get().
                items() if identifier != self._player_identifier])
        else:
            participants_without_player = all_participants
        if self._player_identifier:
            assert all_participants != participants_without_player
        return ConcreteSystemContentForPromptProduct(self._prompt_template.
                                                     format(all_participants=all_participants, dialogue=self.
                                                            _transcription.get(), participants_without_player=
