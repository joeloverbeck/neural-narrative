    ...
    def create_speech_data(self) -> SpeechDataProduct:
        while self._max_retries > 0:
            llm_content_product = (self._llm_content_provider_factory.
                                   create_llm_content_provider(self._messages_to_llm).
                                   generate_content())
            speech_data_product = (self.
                                   _process_llm_content_into_speech_data_strategy.do_algorithm
                                   (llm_content_product))
        ...
        f'Exhausted all retries when trying to get a valid response out of the LLM.'
                                         )
