...
class SpeechTurnDialogueSystemContentForPromptProvider(
    SystemContentForPromptProvider):

    def __init__(self, character: Character, tool_file: str,
    ...
    def create_system_content_for_prompt(self) -> SystemContentForPromptProduct:
        ...
        tool_data['function']['description'] = tool_data['function'][
            'description'].format(**replacements)
        tool_data['function']['parameters']['properties']['narration_text'][
            'description'] = tool_data['function']['parameters']['properties'][
            'narration_text']['description'].format(**replacements)
        ...
        tool_data['function']['parameters']['properties']['speech'][
            'description'] = tool_data['function']['parameters']['properties'][
            'speech']['description'].format(**replacements)
        return ConcreteSystemContentForPromptProduct(self.
                                                     _prompt_formatter_for_dialogue_strategy.do_algorithm() + '\n\n' +
                                                     generate_tool_prompt(tool_data, filesystem_manager.read_file(
                                                         TOOL_INSTRUCTIONS_FILE)), is_valid=True)
