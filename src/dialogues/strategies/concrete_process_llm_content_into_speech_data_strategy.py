...
class ConcreteProcessLlmContentIntoSpeechDataStrategy(ProcessLlmContentIntoSpeechDataStrategy):
    ...
    def do_algorithm(self, llm_content_product: LlmContentProduct) -> SpeechDataProduct:
        ...
        return ConcreteSpeechDataProduct(
            self._speech_tool_response_data_extraction_provider_factory.create_speech_tool_response_data_extraction_provider(
                tool_response_parsing_product).extract_data().get(), is_valid=True)
