...
class ConcreteProcessLlmContentIntoSpeechDataStrategy(
    ...
    def do_algorithm(self, llm_content_product: LlmContentProduct
                     ) -> SpeechDataProduct:
        if not llm_content_product.is_valid():
            return ConcreteSpeechDataProduct({}, is_valid=False, error=
            llm_content_product.get_error())
        tool_response_parsing_product = (self.
                                         _tool_response_parsing_provider_factory.
                                         create_tool_response_parsing_provider(llm_content_product).
                                         parse_tool_response())
...
LLM response content: {llm_content_product.get()}"""
        ...
        self._messages_to_llm.add_message('assistant', llm_content_product.
                                          get())
        speech_turn_response = (self.
        _speech_tool_response_data_extraction_provider_factory.
        create_speech_tool_response_data_extraction_provider(
            tool_response_parsing_product))
        return ConcreteSpeechDataProduct(speech_turn_response.extract_data(
        ).get(), is_valid=True)
