...
class ConcretePromptFormatterForDialogueStrategy(
    PromptFormatterForDialogueStrategy):

    def __init__(self, playthrough_name: str, participants: Participants,
                 purpose: Optional[str], name: str, character_information_factory:
            CharacterInformationProvider, prompt_file: str,
    ...
    def _format_participant_details(self) -> str:
        return '\n'.join([
            ...
            for _, participant in self._participants.get().items() if
            participant['name'] != self._name])
    ...
    def do_algorithm(self) -> str:
        ...
        return self._filesystem_manager.read_file(self._prompt_file).format(
            places_descriptions=self._places_descriptions_factory.
            get_information(), hour=time_manager.get_hour(), time_group=
            time_manager.get_time_of_the_day(), name=self._name,
            participant_details=participant_details, character_information=
            self._character_information_factory.get_information(),
