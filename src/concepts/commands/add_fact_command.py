...
class AddFactCommand(Command):
    ...
    def __init__(self, playthrough_name: str, fact: str, filesystem_manager:
    Optional[FilesystemManager] = None):
    ...
    def execute(self) -> None:
        facts_file_path = self._filesystem_manager.get_file_path_to_facts(self
                                                                          ._playthrough_name)
        if not os.path.exists(facts_file_path):
            self._filesystem_manager.write_file(facts_file_path, None)
