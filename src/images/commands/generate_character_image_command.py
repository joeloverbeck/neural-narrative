...
class GenerateCharacterImageCommand(Command):
    ...
    def __init__(self, playthrough_name: str, character_identifier: str, generated_image_factory: GeneratedImageFactory,
        ...
        self._character_manager = characters_manager or CharactersManager(self._playthrough_name)
    ...
    def execute(self) -> None:
        ...
        url_content_product = self._url_content_factory.get_url(generated_image_product.get())

        if url_content_product.is_valid():
            self._filesystem_manager.write_binary_file(target_image_path, url_content_product.get())
