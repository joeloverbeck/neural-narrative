...
def test_execute_success():
    ...
    random_provider.create_random_place_type_map_entry.return_value = (
        success_result)
    ...
    (map_manager.get_identifier_and_place_template_of_latest_map_entry.
     side_effect) = ([('world_id', 'world_template'), ('region_id',
                                                       'region_template')])
    map_manager_factory = Mock()
    map_manager_factory.create_map_manager.return_value = map_manager
    command = CreateInitialMapCommand(story_universe_template=
                                      story_universe_template,
                                      random_template_type_map_entry_provider_factory=
                                      random_provider_factory, map_manager_factory=map_manager_factory)
    command.execute()
    ...
    assert map_manager.get_identifier_and_place_template_of_latest_map_entry.call_count == 2
...
def test_execute_region_creation_failure():
    ...
    map_manager = Mock()
    (map_manager.get_identifier_and_place_template_of_latest_map_entry.
     return_value) = 'world_id', 'world_template'
    map_manager_factory = Mock()
    map_manager_factory.create_map_manager.return_value = map_manager
    command = CreateInitialMapCommand(story_universe_template=
                                      story_universe_template,
...
def test_execute_area_creation_failure():
    ...
    random_provider = Mock()
    random_provider.create_random_place_type_map_entry.side_effect = [
        success_result, success_result, failure_result]
    random_provider_factory = Mock()
    random_provider_factory.create_provider.return_value = random_provider
    map_manager = Mock()
    (map_manager.get_identifier_and_place_template_of_latest_map_entry.
