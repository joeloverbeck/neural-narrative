...
def test_get_place_full_data_valid_location():
    ...
    # Sample data
    map_file = {
        "loc1": {"type": "location", "place_template": "cave", "area": "area1"},
        "area1": {"type": "area", "place_template": "forest", "region": "region1"},
        "region1": {"type": "region", "place_template": "kingdom"},
    }
    location_templates = {"cave": {"description": "A dark cave"}}
    area_templates = {"forest": {"description": "A lush forest"}}
    region_templates = {"kingdom": {"description": "A vast kingdom"}}
    ...
    filesystem_manager.load_existing_or_new_json_file.side_effect = lambda path: (
        ...
        if "map" in path
            ...
            location_templates
            if "locations_template" in path
            else (
                area_templates
                if "areas_template" in path
    ...
    filesystem_manager.get_file_path_to_areas_template_file.return_value = (
        "areas_template"
    )
    ...
    # Expected result
    expected_result = {
        "location_data": {"name": "cave", "description": "A dark cave"},
        "area_data": {"name": "forest", "description": "A lush forest"},
        "region_data": {"name": "kingdom", "description": "A vast kingdom"},
    }
...
def test_get_place_full_data_valid_region():
    ...
    # Sample data
    map_file = {"region1": {"type": "region", "place_template": "kingdom"}}
    region_templates = {"kingdom": {"description": "A vast kingdom"}}
