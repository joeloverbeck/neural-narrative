...
sample_character_data = {'char1': {'name': 'Test Character', 'description':
    'A test character.', 'personality': 'Friendly', 'profile': 'Warrior',
                                   'likes': 'Adventuring', 'dislikes': 'Laziness', 'secrets': 'None',
                                   'first message': 'Hello there!', 'speech_patterns': 'Formal', 'health':
                                       '100', 'equipment': 'Sword', 'voice_gender': 'Male', 'voice_age':
                                       'Young', 'voice_emotion': 'Neutral', 'voice_tempo': 'Medium',
                                   'voice_volume': 'Medium', 'voice_tone': 'Warm', 'voice_texture':
                                       'Smooth', 'voice_style': 'Narrative', 'voice_personality': 'Calm',
                                   'voice_special_effects': 'None', 'voice_model': 'Model A'}}
...
def test_character_init_with_valid_data(mock_filesystem_manager):
    ...
    identifier = 'char1'
    character = Character(playthrough_name, identifier, filesystem_manager=
...
def test_character_init_with_nonexistent_identifier(mock_filesystem_manager):
    ...
    with pytest.raises(KeyError) as excinfo:
        Character(playthrough_name, identifier, filesystem_manager=
        mock_fs_manager)
    assert f"Character with identifier '{identifier}' not found." in str(
        excinfo)


def test_character_init_with_missing_required_attributes(
        mock_filesystem_manager):
    playthrough_name = 'test_playthrough'
    identifier = 'char1'
    ...
    with pytest.raises(KeyError) as excinfo:
        Character(playthrough_name, identifier, filesystem_manager=
        mock_fs_manager)
    assert f"Character '{identifier}' is missing the following required attributes: name." in str(
        excinfo)
...
def test_update_data_with_allowed_fields(mock_filesystem_manager):
    ...
    character = Character(playthrough_name, identifier, filesystem_manager=
    mock_filesystem_manager)
    updated_data = {'description': 'An updated test character.', 'likes':
        'Exploring'}
...
def test_update_data_with_disallowed_fields(mock_filesystem_manager, caplog):
    ...
    character = Character(playthrough_name, identifier, filesystem_manager=
    mock_filesystem_manager)
    updated_data = {'description': 'An updated test character.',
    ...
    assert any(
        ...
        in message for message in caplog.text.split('\n'))
    mock_filesystem_manager.save_json_file.assert_called()
...
def test_has_description_for_portrait_exists_and_nonempty(
        mock_filesystem_manager):
    playthrough_name = 'test_playthrough'
    identifier = 'char1'
...
def test_image_url_property(mock_filesystem_manager):
    playthrough_name = 'test_playthrough'
    identifier = 'char1'
    (mock_filesystem_manager.get_file_path_to_character_image_for_web.
...
def test_update_data_with_allowed_special_field(mock_filesystem_manager):
    ...
    character = Character(playthrough_name, identifier, filesystem_manager=
    mock_filesystem_manager)
    updated_data = {'image_url': 'http://example.com/new_image.png',
                    'description_for_portrait': 'New portrait description'}
    character.update_data(updated_data)
    assert character.get_attribute('image_url'
                                   ) == 'http://example.com/new_image.png'
