...
def test_clean_json_keys(input_json, expected_output):
    provider = ConcreteToolResponseParsingProvider(RequiredString("dummy"))
...
def test_parse_tool_response_success(
    ...
    provider = ConcreteToolResponseParsingProvider(RequiredString(response))
...
def test_valid_tool_response():
    response = (
        '<function=generate_region>{"name": "Zoopolis", '
        '"description": "A diverse world teeming with sentient species.", '
        '"categories": ["Fantasy World", "Sentient Species"]}</function>'
    )
    provider = ConcreteToolResponseParsingProvider(RequiredString(response))
...
def test_parse_tool_response_valid():
    # The function call string to test
    ...
    # Instantiate the parsing provider
    parsing_provider = ConcreteToolResponseParsingProvider(RequiredString(response))

    # Parse the tool response
    parsed_response = parsing_provider.parse_tool_response()

    # Assert that the response is valid and correctly parsed
...
def test_parse_tool_response_invalid_json():
    # The function call string with invalid JSON
    ...
    # Assert that the response is invalid due to JSON decoding error
...
def test_parse_tool_response_no_function_call():
    # The function call string with no function call
    ...
    # Assert that the response is invalid due to no function call found
    assert parsed_response.is_valid() is False
