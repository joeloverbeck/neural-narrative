...
def setup_dependencies():
    ...
    dialogue_summary_provider_factory.create_dialogue_summary_provider.return_value = dialogue_summary_provider
    ...
    store_character_memory_command_factory = Mock(spec=StoreCharacterMemoryCommandFactory)
    store_memory_command = Mock()
    store_character_memory_command_factory.create_store_character_memory_command.return_value = store_memory_command
...
def test_no_summary_for_insufficient_transcription(setup_dependencies):
    ...
    deps['dialogue_summary_provider_factory'].create_dialogue_summary_provider.assert_not_called()
    deps['store_character_memory_command_factory'].create_store_character_memory_command.assert_not_called()
...
def test_valid_summary_created_and_memories_stored(setup_dependencies):
    ...
    deps['dialogue_summary_provider_factory'].create_dialogue_summary_provider.assert_called_once_with(
        deps['transcription'])
...
def test_summary_creation_fails(setup_dependencies):
    ...
    with pytest.raises(ValueError, match="Failed to create a summary for the dialogue: Invalid summary"):
        command.execute()
